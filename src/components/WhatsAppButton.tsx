...
export const WhatsAppButton = ({ url, label = "¿Dudas?" }: WhatsAppButtonProps) => {
  ...
  return (
    <a
      ...
      className="fixed bottom-[110px] right-4 z-40 group touch-manipulation"
    >
      <div className="flex items-center gap-2 flex-row-reverse">
        ...
        <span className="px-3 py-1.5 rounded-full bg-accent text-accent-foreground text-sm font-semibold shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap pointer-events-none">
          {label}
        </span>
      </div>
    </a>
