...
import { Trash2, Tag, X, ChevronDown, ChevronUp, ShoppingBag, Truck, Zap } from "lucide-react";
...
export function OrderSummary({ collapsible = false, locked = false, mainProductId, onExpandedChange }) {
    const { items, coupon, couponPercent, updateQuantity, removeItem, applyCoupon, removeCoupon, selectedMethod } = useCheckoutPruebaStore();
    ...
    const localItemsSum = sumItemsLocal(items.map((i) => ({ id: i.id, usd: itemPrice(i, region.tier), quantity: i.quantity || 1 })), region.country || "", overridesFor);
    ...
    const fmtMoney = (usd, _local, penAmount) => penMode && penAmount != null
        ? formatPen(penAmount)
        : showLocalRef
            ? _local.formatted
    ...
    return (<div className="bg-muted/30 lg:bg-muted/50 rounded-xl border overflow-hidden">
      {collapsible && (<button onClick={() => {
                const next = !expanded;
                setExpanded(next);
                onExpandedChange?.(next);
            }} className="w-full flex items-center justify-between px-4 py-3 lg:hidden bg-primary/5 hover:bg-primary/10 transition-colors border-b" aria-expanded={expanded}>
          <span className="flex items-center gap-2.5 text-sm font-medium">
            <span className="relative">
              <ShoppingBag className="w-5 h-5 text-primary"/>
              {itemCount > 0 && (<span className="absolute -top-1.5 -right-1.5 bg-primary text-primary-foreground text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center">
                  {itemCount}
                </span>)}
            </span>
          ...
          <span className="text-right flex flex-col items-end">
            ...
            {showLocalRef && (<span className="text-[10px] font-normal text-muted-foreground leading-none">
      ...
      <div className={cn("p-5 space-y-4", collapsible && !expanded && "hidden lg:block", collapsible && expanded && "lg:max-h-none max-h-[calc(100vh-160px)] overflow-y-auto")}>
        ...
        {items.some(i => i.isPhysical) ? (<div className="flex flex-col gap-2 rounded-xl border border-destructive/25 bg-destructive/5 px-3 py-2.5 mb-2 ring-1 ring-destructive/20">
            ...
            <div className="text-[10px] space-y-1 pl-6 opacity-90 italic">
              ...
              {isAsia && (<div className="space-y-1">
              ...
              {!isLatam && !isNorthAmericaEurope && !isAsia && (<p className="text-destructive">{t.shippingNoticeGlobal}</p>)}
            </div>
          </div>) : (<div className="flex gap-2.5 items-start rounded-xl border border-primary/25 bg-primary/5 px-3 py-2.5 mb-2">
        ...
        {items.length === 0 ? (<div className="space-y-3 py-2">
            ...
            {items.map((item) => (<div key={item.id} className="flex gap-3 items-start">
                <div className="relative shrink-0">
                  <img src={item.image} alt={item.name} className="w-16 h-16 object-cover rounded-lg border" loading="lazy"/>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate">{item.name}</p>
                  {item.description && (<p className="text-xs text-muted-foreground truncate">{item.description}</p>)}
                  {!locked && item.id !== mainProductId && (<div className="flex items-center justify-end mt-1.5">
                      <Button type="button" variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground hover:text-destructive" onClick={() => removeItem(item.id)} aria-label="Quitar">
        ...
        <div className="border-t pt-4 space-y-2">
          {coupon ? (<div className="flex items-center justify-between bg-primary/10 text-primary text-sm rounded-md px-3 py-2">
              <span className="flex items-center gap-1.5 font-medium">
                <Tag className="w-3.5 h-3.5"/>
                {coupon} · -{couponPercent}%
              </span>
              <button onClick={removeCoupon} type="button" className="opacity-70 hover:opacity-100" aria-label={t.removeCoupon}>
                <X className="w-4 h-4"/>
              </button>
            </div>) : (<div>
              <div className="flex gap-2">
                <Input placeholder={t.couponPlaceholder} value={couponInput} onChange={(e) => {
                setCouponInput(e.target.value);
                setCouponError(null);
            }} maxLength={20} onKeyDown={(e) => e.key === "Enter" && handleApplyCoupon()}/>
                <Button type="button" variant="secondary" onClick={handleApplyCoupon} disabled={!couponInput.trim()}>
                  {t.applyCoupon}
                </Button>
              </div>
        ...
        <div className="border-t pt-4 space-y-2 text-sm">
          ...
          <div className="flex justify-between items-baseline text-base font-bold pt-2 border-t">
            ...
            <div className="text-right">
              ...
              {showLocalRef && (<div className="text-[11px] text-muted-foreground font-normal mt-0.5">
                  ≈ USD ${localItemsSum.usdReference.toFixed(2)}
                </div>)}
              
              {showLocalRef && !breakdown.isUsd && (<div className="mt-4 pt-4 border-t border-dashed space-y-2">
                  ...
                  {breakdown.hasRegionalUsd && (<div className="flex justify-between text-xs text-green-600 font-medium bg-green-50/50 px-2 py-1 rounded">
                  ...
                  {breakdown.hasRegionalUsd && (<p className="text-[9px] text-muted-foreground italic leading-tight mt-1">
