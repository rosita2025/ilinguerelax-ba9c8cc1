...
function LogoBadge({ src, alt, bg = "#ffffff" }: { src: string; alt: string; bg?: string }) {
  return (
    <span
      className="inline-flex items-center justify-center h-8 w-12 rounded-md border border-neutral-200 dark:border-neutral-700 shadow-sm"
      style={{ background: bg }}
    >
      <img src={src} alt={alt} className="max-h-5 max-w-[38px] object-contain" />
    </span>
  );
}
...
function LinkBadge() {
  return (
    ...
    >
      <span className="text-[12px] font-bold tracking-tight" style={{ color: "#0A2540" }}>
        Link
      </span>
    </span>
  );
