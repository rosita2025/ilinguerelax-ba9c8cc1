 ...
 */
export type FamilyKey = "stripe" | "stripeAch" | "stripeCashApp" | "stripeKlarna" | "paypal" | "transfer" | "cash" | "yape" | "binance" | "clabe" | "hotmart" | "dlocalTransfer" | "dlocalCash" | "dlocalWallet" | "dlocalCard";
...
function keyToFamily(key: string): FamilyKey | null {
  ...
  if (k === "dlocal_card" || k === "dlocal_smartfields") return "dlocalCard";
...
export function useCheckoutMethodsConfig(country: string): CheckoutMethodsConfig {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!region) {
        // Fallback robusto: si no hay región configurada, Stripe siempre debe estar disponible para pagos internacionales.
        if (alive) setState({ 
          loaded: true, 
          regionCode: "GLOBAL", 
      ...
      const enabledFamilies = { ...DEFAULT_ALL_OFF, stripe: true };
      const familyMinOrder: Record<FamilyKey, number> = { stripe: -1, stripeAch: Infinity, stripeCashApp: Infinity, stripeKlarna: Infinity, paypal: 0, transfer: Infinity, cash: Infinity, yape: Infinity, binance: Infinity, clabe: Infinity, hotmart: Infinity, dlocalTransfer: Infinity, dlocalCash: Infinity, dlocalWallet: Infinity, dlocalCard: Infinity };

      const enabledMethodKeys: string[] = ["stripe_card", "paypal"];
      enabledFamilies.paypal = true;
