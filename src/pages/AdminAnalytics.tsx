        ...
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          {!data ? (
            ...
            <>
              ...
              {/* FX rates transparency */}
              {data.fx && (
                <Card className="p-4">
                  <div className="flex items-center justify-between flex-wrap gap-2 mb-3">
                    <div>
                      <h2 className="font-semibold flex items-center gap-2">
                        <DollarSign className="w-4 h-4 text-primary" /> Tipo de cambio (base USD)
                      </h2>
                      <p className="text-xs text-muted-foreground">
                        Fuente: <span className="font-mono">{data.fx.source}</span>
                        {" · "}Publicado: {format(new Date(data.fx.fetchedAt), "d MMM yyyy HH:mm 'UTC'", { locale: es })}
                        {" · "}Calculado: {format(new Date(data.fx.computedAt), "d MMM yyyy HH:mm 'UTC'", { locale: es })}
                      </p>
                    </div>
                  </div>

                  {data.fx.pendingByCurrency.length === 0 ? (
                    <p className="text-sm text-muted-foreground">
                      No hay montos pendientes en monedas distintas de USD para este rango.
                    </p>
                  ) : (
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead className="text-xs text-muted-foreground border-b">
                          <tr>
                            <th className="text-left py-2 pr-3">Moneda</th>
                            <th className="text-right px-2">Tasa (1 USD =)</th>
                            <th className="text-right px-2">Inversa (1 = USD)</th>
                            <th className="text-right px-2">Pendiente</th>
                            <th className="text-right pl-2">≈ USD</th>
                          </tr>
                        </thead>
                        <tbody>
                          {data.fx.pendingByCurrency.map((r) => (
                            <tr key={r.currency} className="border-b border-border/40 hover:bg-muted/40">
                              <td className="py-2 pr-3 font-medium">
                                {r.currency}
                                <div className="text-[11px] text-muted-foreground">
                                  {r.breakdown.map((b) => `${b.source === "hotmart" ? "Hotmart" : "Tienda"}: ${b.count}`).join(" · ")}
                                </div>
                              </td>
                              <td className="text-right px-2 tabular-nums">
                                {r.rate ? r.rate.toLocaleString("en-US", { maximumFractionDigits: 4 }) : "—"}
                              </td>
                              <td className="text-right px-2 tabular-nums">
                                {r.rateInverse ? r.rateInverse.toLocaleString("en-US", { maximumFractionDigits: 6 }) : "—"}
                              </td>
                              <td className="text-right px-2 tabular-nums">
                                {r.amount.toLocaleString("en-US", { maximumFractionDigits: 2 })} {r.currency}
                              </td>
                              <td className="text-right pl-2 tabular-nums font-semibold">
                                {money(r.usdEquivalent)}
                              </td>
                            </tr>
                          ))}
                          <tr className="bg-muted/30">
                            <td className="py-2 pr-3 font-semibold" colSpan={4}>Total pendiente estimado</td>
                            <td className="text-right pl-2 tabular-nums font-bold">
                              {money(data.fx.pendingByCurrency.reduce((s, r) => s + r.usdEquivalent, 0))}
                            </td>
                          </tr>
                        </tbody>
                      </table>
                  ...
                  <details className="mt-3">
                    <summary className="text-xs text-muted-foreground cursor-pointer hover:text-foreground">
                      Ver tasas de referencia usadas ({Object.keys(data.fx.rates).length} monedas)
                    </summary>
                    <div className="mt-2 grid grid-cols-2 md:grid-cols-4 gap-1 text-[11px] font-mono">
                      {["MXN","BRL","ARS","COP","CLP","PEN","EUR","GBP","CAD"]
                        .filter((c) => data.fx!.rates[c])
                        .map((c) => (
                          <div key={c} className="flex justify-between border border-border/50 rounded px-2 py-1">
                            <span>{c}</span>
                            <span className="tabular-nums">{data.fx!.rates[c].toFixed(3)}</span>
                          </div>
                        ))}
                    </div>
                  </details>
