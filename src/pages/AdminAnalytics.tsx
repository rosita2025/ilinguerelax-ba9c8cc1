...
const normalizeAnalyticsData = (value: Partial<AnalyticsData> | null | undefined, fallbackRange: ReturnType<typeof rangeForPreset>): AnalyticsData => {
  ...
  return {
    ...
    pendingOrders: toArray(value?.pendingOrders).slice(0, 100).map((p) => ({
      orderNumber: toText(p?.orderNumber, "-"),
      provider: toText(p?.provider, "otros"),
      source: toText(p?.source, "store"),
      status: toText(p?.status, "pending"),
      email: toText(p?.email, ""),
      country: toText(p?.country, "??"),
      product: toText(p?.product, ""),
      amount: toNumber(p?.amount),
      currency: toText(p?.currency, "USD"),
      amountUsd: toNumber(p?.amountUsd),
      createdAt: toText(p?.createdAt, ""),
      lastCheckAt: p?.lastCheckAt ? toText(p.lastCheckAt, "") : null,
    })),
    conversion: {
