...
import { Lock, ShieldCheck, MessageCircle, ArrowLeft, BadgeCheck, Package } from "lucide-react";
...
import { getCatalogItem, resolveCheckoutSlug, CHECKOUT_CATALOG } from "@/config/checkoutCatalog";
...
function MobileOrderSummarySticky({ slug }) {
    const [expanded, setExpanded] = useState(false);
    return (<div className={cn("lg:hidden z-20 bg-background/95 backdrop-blur border-b", expanded ? "relative" : "sticky top-[44px] sm:top-[52px]")}>
...
export default function Checkout() {
    ...
    const [adminUpsells, setAdminUpsells] = useState(null);
    ...
    const mergedFromStatic = staticItem
        ...
        toast.info(language === "en"
            ...
            : language === "fr"
                ...
                : language === "pt"
                    ...
                    : `“${catalogItem.name}” se agregó de nuevo a tu carrito. Este producto no se puede quitar desde aquí.`, { duration: 4500 });
    ...
    useEffect(() => {
        ...
        if (!sku || initiatedRef.current === sku)
            return;
        initiatedRef.current = sku;
        const tier = (region.tier || "global");
        const priceForTier = tier === "peru" && catalogItem.pricePen != null
            ? Number(catalogItem.pricePen)
            : Number(catalogItem.regionPrices?.[tier === "peru" ? "latam" : tier] ??
                catalogItem.price);
    ...
    if (slugUnknown) {
        ...
        <header className="border-b bg-background/95 backdrop-blur sticky top-0 z-30">
          <div className="max-w-6xl mx-auto px-3 sm:px-4 py-2.5 sm:py-3 flex items-center justify-between gap-2">
            <span className="text-base sm:text-xl font-bold tracking-tight whitespace-nowrap" style={{ fontFamily: "Plus Jakarta Sans, sans-serif" }}>
    ...
    return (<div className="min-h-screen bg-background overflow-x-hidden max-w-full">
      {loadingDb && !catalogItem && (<div className="max-w-6xl mx-auto px-3 sm:px-4 py-3 sm:py-5 lg:py-8 space-y-6">
      ...
      <header className="border-b bg-background/95 backdrop-blur sticky top-0 z-30">
        <div className="max-w-6xl mx-auto px-3 sm:px-4 py-2.5 sm:py-3 flex items-center justify-between gap-2">
          <div className="flex items-center gap-3 min-w-0">
            {catalogItem?.productPath && (<Link to={catalogItem.productPath} className="flex items-center gap-1 text-[11px] sm:text-xs text-muted-foreground hover:text-primary whitespace-nowrap" aria-label={t.returnToProduct || "Volver al producto"}>
      ...
      <div className="max-w-6xl min-w-0 mx-auto px-3 sm:px-4 py-3 sm:py-5 lg:py-8 grid lg:grid-cols-[minmax(0,1fr)_400px] gap-5 lg:gap-8">
        <div className="min-w-0 space-y-4">
          {items.some(i => i.isPhysical) && items.some(i => !i.isPhysical) && (<div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-900/30 rounded-xl p-4 flex gap-3 items-start">
          ...
          {catalogItem?.upsells?.length ? (<SectionErrorBoundary name="upsell-panel" extra={{ slug: catalogItem?.id }}>
              <UpsellPanel upsells={catalogItem.upsells} mainProductId={catalogItem.id}/>
            </SectionErrorBoundary>) : null}
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 text-xs text-muted-foreground pt-2">
            ...
            <a href="https://wa.me/112512724704" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 hover:text-primary">
