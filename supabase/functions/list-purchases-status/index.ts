  ...
  try {
    ...
    if (!provider || provider === "hotmart") {
      const { data } = await admin
        ...
        .or("provider.eq.hotmart,referrer.ilike.%hotmart-webhook%,referrer.ilike.%\"provider\":\"hotmart\"%,event_name.ilike.purchase%,session_id.ilike.HP%,referrer.ilike.%\"hottok\":%,referrer.ilike.%HP1413567978%")
        .order("created_at", { ascending: false })
        .limit(take);
